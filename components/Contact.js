...
const ContactForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ FILE DIRECT APPEND KAREN
      if (profileImage) {
        formDataToSend.append('profileImage', profileImage);
      }

      const response = await fetch('https://porthfolio-backend-1.onrender.com/api/contact/submit', {
        method: 'POST',
        body: formDataToSend // ✅ Content-Type header automatically set hoga
      });
      ...
      if (data.success) {
        setFormStatus({ 
          type: 'success', 
          message: '✅ Your message has been sent successfully! I might feature your feedback in my testimonials.' 
        });
        setFormData({ name: '', email: '', subject: '', message: '' });
        setProfileImage(null);
        setPreviewImage(null);
